...
export const ParticleHero: React.FC<ParticleHeroProps> = ({
  ...
  return (
    ...
    >
      {/* Navigation Menu */}
      {navItems.length > 0 && (
        <nav className="absolute top-0 left-0 z-20 p-6 md:p-8">
          <ul className="flex flex-col gap-3">
            {navItems.map((item, index) => (
              <li key={index}>
                <button
                  onClick={item.onClick}
                  className="group flex items-center gap-3 text-red-200/70 hover:text-white transition-all duration-300"
                >
                  <span className="inline-block w-3 h-3 bg-red-500 group-hover:bg-red-400 transition-colors duration-300 rotate-45 group-hover:scale-110" />
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6">
        {children ? (
          ...
          <div className="text-center max-w-6xl mx-auto">
            ...
            <div className="mb-16">
              <h1
                className="text-8xl md:text-[10rem] lg:text-[14rem] xl:text-[16rem] tracking-tight leading-[0.9] mb-8"
                style={{ fontFamily: "var(--font-faster-one), serif" }}
              >
